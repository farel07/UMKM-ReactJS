...
export function HomePage({ onDetailClick }: HomePageProps) {
  ...
  // Fungsi untuk handle click dengan scroll to top
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-blue-950 relative overflow-hidden">
        ...
        <motion.div style={{ opacity }} className="relative max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div variants={heroContainerVariants} initial="hidden" animate="visible">
              <motion.div variants={heroItemVariants} className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-500/20 to-purple-500/20 backdrop-blur-sm px-4 py-2 rounded-full border border-white/10 mb-6">
                <Sparkles className="size-4 text-yellow-400" />
                <span className="text-white/90">Platform UMKM Ketintang & Sekitarnya</span>
              </motion.div>
              <motion.h1 variants={heroItemVariants} className="text-white mb-6">
              ...
              <motion.p variants={heroItemVariants} className="text-white/70 mb-8 max-w-xl">
      ...
      <section ref={umkmSectionRef} className="relative max-w-7xl mx-auto px-6 pb-20">
        <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{ duration: 0.5 }}>
            ...
            <div className="flex gap-2 flex-wrap">
              {categories.map((category) => (
                <motion.button
                  ...
                  whileHover={{ y: -2 }}
                  whileTap={{ scale: 0.95 }}
                  className={`px-4 py-2 rounded-xl transition-colors duration-200 ${selectedCategory === category ? "bg-white/20 text-white border border-white/30" : "bg-white/5 text-white/60 hover:bg-white/10 border border-white/10"}`}
                >
                  {category}
                </motion.button>
              ))}
          ...
          <motion.div layout className="grid grid-cols-12 gap-4 auto-rows-[280px]">
            <AnimatePresence>
              {filteredUMKM.length > 0 ? (
                filteredUMKM.map((umkm, index) => {
                  ...
                  return <UMKMCard key={umkm.id} umkm={umkm} onDetailClick={handleDetailClick} index={index} className={pattern} />;
                })
              ) : (
                <motion.div
                  key="no-results"
                  ...
                  className="col-span-12 text-center py-20 bg-white/5 backdrop-blur-sm rounded-3xl border border-white/10"
                >
